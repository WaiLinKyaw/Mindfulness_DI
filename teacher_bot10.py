...
# Global သို့မဟုတ် Bot စတင်ချိန်တွင် တစ်ကြိမ်တည်းသာ ဖန်တီးထားပါ
...
"""
    ...
    # Connection Reset မဖြစ်စေရန် client အသစ်ဖြင့် စတင်ခြင်း
    ...
    for model_name in candidate_models:
        for attempt in range(3):
            try:
                ...
                # [WinError 10054] ကြုံပါက Client အသစ်ဖွင့်ပြီး ပြန်စမ်းခြင်း
                wait_sec = (attempt + 1) * 2
                print(f"⚠️ [WinError 10054/Network] on {model_name}. Retrying in {wait_sec}s... ({net_err})")
                await asyncio.sleep(wait_sec)
                client = genai.Client(api_key=api_key)

            except APIError as api_err:
                # 503 (Busy) သို့မဟုတ် 429 (Rate Limit) အတွက် စောင့်ဆိုင်းခြင်း
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != TEACHER_CHAT_ID and update.effective_user.id != DEVELOPER_CHAT_ID:
    ...
    text = (
        ...
        ". `/send_routine morning/afternoon/evening`  ကျောင်းသားအားလုံးသို့ပို့ရန်"
        ". `/detail_report <student_id>      `/ai_report <student_id>`"
    )
...
async def reply_to_student(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != TEACHER_CHAT_ID and user_id != DEVELOPER_CHAT_ID:
...
async def get_student_task_breakdown(conn, student_id: int, start_date, end_date):
    """ကျောင်းသား၏ Task တစ်ခုချင်းစီအလိုက် ပြီးမြောက်မှု အကြိမ်ရေကို စစ်ဆေးခြင်း"""
    query = """
        SELECT 
    ...
    """
    rows = await conn.fetch(query, student_id, start_date, end_date)
    return rows
    


def format_plain_detail_report(student_name: str, task_summary: list, start_date, end_date) -> str:
    """AI မပါဘဲ ကျောင်းသား၏ အလုပ်ပြီးစီးမှု အသေးစိတ်ကို Format ပြုလုပ်ခြင်း"""
    total_assigned = 0
    total_done = 0
...
"""
    ...
    # 503 တက်ပါက လဲလှယ်သုံးမည့် Models အစဉ်လိုက်
    candidate_models = ["gemini-3.6-flash", "gemini-3.5-flash", "gemini-3.5-flash-lite"]

    for model_name in candidate_models:
        # Model တစ်ခုစီအတွက် 3 ကြိမ်အထိ စက္ကန့်ပိုင်းခြားပြီး ထပ်ကြိုးစားခြင်း
        for attempt in range(3):
            try:
                ...
                # 503 သို့မဟုတ် 429 ဖြစ်ပါက ခေတ္တစောင့်ပြီး ပြန်လည်ကြိုးစားရန်
                if e.code in (503, 429):
                    wait_sec = (attempt + 1) * 3
                    print(f"⚠️ {model_name} busy (Code {e.code}). Retrying in {wait_sec}s... (Attempt {attempt+1}/3)")
                    await asyncio.sleep(wait_sec)
                else:
                    raise e
            except Exception as e:
    ...
    # အားလုံး မရပါက fallback စာသား ထုတ်ပေးခြင်း
...
async def send_detailed_weekly_report(application, student_id: int):
    ...
    async with db_pool.acquire() as conn:
        # first_name အစား student_name သို့ ပြောင်းလဲခြင်း
...
async def send_weekly_report(app, manual_chat_id=None):
    ...
    target_chat_id = manual_chat_id or TEACHER_CHAT_ID or DEVELOPER_CHAT_ID
...
# (၂) Detail Report With AI
async def send_detailed_report_with_ai(application, student_id: int, recipient_chat_id: int = None):
    """Data အပြင် Gemini AI ၏ သုံးသပ်နားချချက်ပါ ထည့်သွင်းထားသော အစီရင်ခံစာ ပို့ဆောင်ခြင်း"""
    target_chat = recipient_chat_id or student_id
    today = datetime.now(MM_TZ).date()
    start_date = today - timedelta(days=7)
...
async def cmd_detail_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ဆရာ့ Chat သို့ တိုက်ရိုက် အဖြေပို့ရန် recipient_chat_id ထည့်ထားပါသည်
...
async def broadcast_detailed_weekly_reports(application):
    """တနင်္ဂနွေနေ့တွင် ကျောင်းသားအားလုံးထံ AI အသေးစိတ် Report ပို့ဆောင်ခြင်း"""
    ...
    for stu in students:
        ...
        try:
            ...
            await asyncio.sleep(2)  # Rate limit ကာကွယ်ရန်
        except Exception as e:
...
# Section အလိုက် ဖော်ပြမည့် မူရင်း Header စာသားများ
SECTION_HEADERS = {
...
async def cmd_send_routine(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ဆရာက ကျောင်းသားအားလုံး (သို့မဟုတ် တစ်ဦးတည်းထံ) Routine လှမ်းပို့သည့် Command"""
    user_id = update.effective_user.id

    # ၁။ ဆရာ ဟုတ်/မဟုတ် စစ်ဆေးခြင်း (ဆရာမဟုတ်ပါက ခွင့်မပြုပါ)
    if user_id != TEACHER_CHAT_ID and user_id != DEVELOPER_CHAT_ID:
        await update.message.reply_text("⛔ ဤ Command ကို အသုံးပြုရန် ခွင့်ပြုချက်မရှိပါ။")
        return

    # ၂။ Argument စစ်ဆေးခြင်း
    if not context.args:
        help_text = (
            "📌 *အသုံးပြုပုံ လမ်းညွှန်:*\n\n"
            "၁။ ကျောင်းသားအားလုံးထံ ပို့လိုပါက:\n"
            "   `/send_routine <morning|afternoon|evening>`\n"
            "   ဥပမာ: `/send_routine morning`\n\n"
            "၂။ ကျောင်းသားတစ်ဦးတည်းထံ စမ်းသပ်ပို့လိုပါက:\n"
            "   `/send_routine <section> <student_id>`\n"
            "   ဥပမာ: `/send_routine morning 12345678`"
        )
    ...
    if section not in SECTION_HEADERS:
        await update.message.reply_text(
            "⚠️ Section အမည် မှားယွင်းနေပါသည်။ `morning`, `afternoon`, သို့မဟုတ် `evening` သာ ရိုက်ထည့်ပေးပါခင်ဗျာ။",
            parse_mode="Markdown"
        )
        return

    # ကျောင်းသား ID ပါဝင်မှု ရှိ/မရှိ စစ်ဆေးခြင်း
    ...
    # ၃။ ပို့ဆောင်နေကြောင်း အသိပေး Message ပို့ထားခြင်း
    ...
    status_msg = await update.message.reply_text(
        f"⏳ {target_label} *{section}* routine ပို့ဆောင်နေပါသည်...",
        parse_mode="Markdown"
    )

    try:
        # User ရေးထားသော broadcast_routine_to_students function ကို လှမ်းခေါ်ခြင်း
        ...
        if sent_count > 0:
            await status_msg.edit_text(
                f"✅ *{section.capitalize()} Routine ပို့ဆောင်ပြီးပါပြီ!*\n\n"
                f"👥 စုစုပေါင်း လက်ခံရရှိသူ: *{sent_count}* ဦး",
                parse_mode="Markdown"
            )
        else:
            await status_msg.edit_text(
                f"⚠️ ပို့ဆောင်၍ မရပါ (သက်ဆိုင်ရာ Section တွင် Task မရှိသေးခြင်း သို့မဟုတ် ကျောင်းသားစာရင်း မရှိခြင်း ဖြစ်နိုင်ပါသည်)။"
            )
    except Exception as e:
...
async def cmd_check_jobs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ဆရာက /jobs ဟု ရိုက်ပါက လက်ရှိ Schedule အားလုံးကို ပြသပေးခြင်း"""
    if update.effective_user.id != TEACHER_CHAT_ID and update.effective_user.id != DEVELOPER_CHAT_ID:
        return

    jobs = scheduler.get_jobs()
    # job.id ပါ ပေါ်လာစေရန် ထည့်သွင်းခြင်း
    print(f"👉 [ID: {job.id}] Job: {job.name} | Next Run: {job.next_run_time}")
    if not jobs:
    ...
    for job in jobs:
        text += f"🔹 *ID:* `{job.id}`\n"
        text += f"   *အမည်:* {job.name}\n"
        text += f"   *နောက်တစ်ကြိမ် ပို့မည့်အချိန်:* `{job.next_run_time}`\n\n"
...
async def notify_with_three_beeps(chat_id: int, reminder_text: str):
    """Notification sound ၃ ကြိမ် မြည်စေပြီး အဓိက Reminder စာသား ပို့ဆောင်ခြင်း"""
    # အသံ ၂ ကြိမ် မြည်စေရန် message တို ၂ ခု ပို့ပြီး ချက်ချင်း ပြန်ဖျက်ခြင်း
    for _ in range(2):
        try:
    ...
    # တတိယအကြိမ်တွင် အသံနှင့်အတူ အဓိက Reminder စာသား ပို့ခြင်း
    try:
        await student_bot.send_message(
            chat_id=chat_id,
            text=reminder_text,
...
async def send_section_pending_reminder(application, section_name: str, reminder_title: str):
    """မနက် ၁၁:၀၀ နှင့် နေ့လယ် ၄:၀၀ အတွက် သက်ဆိုင်ရာ Section အလိုက် အလုပ်ကျန်သူများကို သတိပေးခြင်း"""
    today = datetime.now(MM_TZ).date()
    ...
    async with db_pool.acquire() as conn:
        ...
        # sdl.template_id = rt.id သို့ ပြင်ဆင်ထားပါသည်
        query = """
        SELECT 
            s.student_id, 
            s.student_name,
...
async def send_evening_all_pending_summary(application):
    """ည ၈:၁၅ တွင် မနက်၊ နေ့လယ်၊ ည အလုပ်ကြွေးအားလုံးကို အရေအတွက်ဖြင့်သာ အကျဉ်းချုပ် သတိပေးခြင်း"""
    today = datetime.now(MM_TZ).date()

    async with db_pool.acquire() as conn:
        active_tasks = await conn.fetch(
            "SELECT id FROM routine_templates WHERE status = 'active';"
        )
        if not active_tasks:
            return
...
async def cmd_remind_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ဆရာက Reminder စနစ်ကို လက်ဖြင့် စမ်းသပ် run သည့် command"""
    if update.effective_user.id != TEACHER_CHAT_ID and update.effective_user.id != DEVELOPER_CHAT_ID:
        await update.message.reply_text("⛔ ဤ Command ကို အသုံးပြုရန် ခွင့်ပြုချက်မရှိပါ။")
        return
...
async def post_init(application):
    ...
    scheduler.add_job(
        ...
        CronTrigger(hour=5, minute=00, timezone=MM_TZ),
    ...
    scheduler.add_job(
        broadcast_routine_to_students,
        CronTrigger(hour=12, minute=00, timezone=MM_TZ),
    ...
    scheduler.add_job(
        broadcast_routine_to_students,
        CronTrigger(hour=17, minute=00, timezone=MM_TZ),
    ...
    # (င) တနင်္ဂနွေနေ့တိုင်း ည ၈:၃၀ နာရီတွင် ကျောင်းသားတစ်ဦးချင်းစီထံ အသေးစိတ် AI Report ပို့မည်
    scheduler.add_job(
        broadcast_detailed_weekly_reports,
        CronTrigger(day_of_week="sun", hour=20, minute=30, timezone=MM_TZ),
        args=[application],
    ...
    # ၁။ မနက် ၁၁:၀၀ နာရီ Reminder
    scheduler.add_job(
        send_section_pending_reminder,
        CronTrigger(hour=11, minute=0, timezone=MM_TZ),
    ...
    # ၂။ နေ့လယ် ၄:၀၀ နာရီ Reminder
    scheduler.add_job(
        send_section_pending_reminder,
        CronTrigger(hour=16, minute=0, timezone=MM_TZ),
    ...
    # ၃။ ည ၈:၁၅ နာရီ Reminder (အလုပ်ကြွေး အရေအတွက် အကျဉ်းချုပ်)
    scheduler.add_job(
        send_evening_all_pending_summary,
        CronTrigger(hour=20, minute=15, timezone=MM_TZ),
        args=[application],
...
def main():
   ...
   # app.add_handler(CommandHandler("detail_report", manual_detailed_report_command))
