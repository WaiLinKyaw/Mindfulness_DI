...
async def broadcast_routine_to_students(section_name: str, header_text: str, target_student_id: int = None) -> int:
    """Task တစ်ခုချင်းစီကို သက်ဆိုင်ရာ Done ခလုတ်ဖြင့် တစ်ကတ်ချင်းစီ ခွဲပို့ပေးခြင်း"""
    today = datetime.now(MM_TZ).date()
    async with db_pool.acquire() as conn:
    ...
    async with Bot(token=STUDENT_BOT_TOKEN) as student_bot:
        for stu in students:
            ...
            try:
                # ၁။ အစပိုင်းတွင် ခေါင်းစဉ် (Header) ကို အရင်ပို့ခြင်း
                await student_bot.send_message(
                    chat_id=chat_id,
                ...
                # ၂။ Task တစ်ခုချင်းစီကို စာသား + ခလုတ် တွဲလျက် ပို့ခြင်း
                ...
                print(f"Error sending individual tasks to {chat_id}: {e}")
...
async def post_init(application):
    ...
    # 1. Database ချိတ်ဆက်ခြင်း
    await init_db()

    # 2. Scheduler ကို active event loop ထဲတွင် စတင်ခြင်း
    ...
    scheduler.start()
    print("⏰ Weekly Report Scheduler started (Sundays at 9:00 PM MMT)...")
