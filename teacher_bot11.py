...
# ဆရာနှင့် Developer နှစ်ဦးလုံးအတွက် Admin IDs စာရင်း
...
async def broadcast_routine_to_students(section_name: str, header_text: str, target_student_id: int = None) -> int:
    ...
    async with db_pool.acquire() as conn:
        ...
        tasks = await conn.fetch(
            "SELECT id, title, description FROM routine_templates WHERE section = $1 AND status = 'active' AND is_active = TRUE ORDER BY task_order ASC;",
            section_name
        )
