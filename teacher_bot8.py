...
၃။ ဖတ်ရလွယ်ကူအောင် စာပိုဒ်တိုများ၊ Bullet ပုံစံများနှင့် သင့်တော်သော Emoji များကို သုံးပေးပါ။
"""
...
async def send_detailed_report_with_ai(application, student_id: int, recipient_chat_id: int = None):
    """Data နှင့် AI အကြံပြုချက်ကို Message ၂ စောင် ခွဲ၍ စာလုံးရေ ကန့်သတ်ချက် မကျော်အောင် ပို့ဆောင်ခြင်း"""
    ...
    # ၁။ ပထမ မက်ဆေ့ခ်ျ - အချက်အလက်ဇယားကို သီးသန့် အရင်ပို့ခြင်း
    ...
    # ၂။ Gemini AI ထံမှ အကြံပြုချက် ထုတ်ယူခြင်း
    ai_feedback = await generate_personalized_ai_report(student_name, task_summary)

    # ၃။ ဒုတိယ မက်ဆေ့ခ်ျ - AI အကြံပြုချက်ကို သီးသန့် ဒုတိယ မက်ဆေ့ခ်ျအဖြစ် ပို့ခြင်း
    ai_message = f"🤖 *ဆရာ့ထံမှ အကြံပြု သုံးသပ်ချက်*\n━━━━━━━━━━━━━━━━━━━━\n{ai_feedback}"
    
    # AI စာသားကိုယ်တိုင် 4096 ကျော်သွားနိုင်ခြေအတွက်ပါ Safe Chunking ပြုလုပ်ခြင်း
    if len(ai_message) > 4000:
        for i in range(0, len(ai_message), 4000):
            await application.bot.send_message(chat_id=target_chat, text=ai_message[i:i+4000])
            await asyncio.sleep(0.5)
    else:
        await application.bot.send_message(chat_id=target_chat, text=ai_message)
